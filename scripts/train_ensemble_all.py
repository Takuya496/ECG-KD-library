"""
② KD-Ensemble 全被験者学習（検証なし・デプロイ用モデル保存）
...
RESULT_PATH        = '/mnt/learn/usr/hayashi/引継ぎ/result/ECG_KD_student_ensemble_allsubjects/'
...
with strategy.scope():
    ...
    teacher_ckpt.restore(tf.train.latest_checkpoint(TEACHER2_CKPT)).expect_partial()
    teacher_reg_head.trainable = False
    print('教師②（KDアンサンブル）をロードしました')
...
checkpoint_prefix = os.path.join(RESULT_PATH, 'model_ECG_KD_ensemble')
